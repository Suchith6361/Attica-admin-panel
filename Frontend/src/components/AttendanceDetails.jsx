  ...
  return (
    <div className="p-6 max-w-7xl mx-auto bg-gray-100 rounded-lg shadow-lg mt-20 md:ml-[275px]">
      ...
      <div className="flex flex-col md:flex-row justify-center space-x-0 md:space-x-4 mb-6">
        <button
          onClick={() => navigate(`/employees/${employeeId}/attendance-list/leaves`)}
          className="px-6 py-2 bg-blue-500 text-white rounded-lg shadow hover:bg-blue-600 transition duration-300 mb-2 md:mb-0"
        >
          View Leave Requests
        </button>
        <button
          onClick={() => navigate(`/employees/${employeeId}/attendance-list/complaints`)}
      ...
      {loading ? (
        ...
        <div className="mb-8">
          {filteredAttendance.length > 0 ? (
            ...
            <p className="text-center text-gray-500 mt-4">No attendance records found for this employee.</p>
          )}
        </div>
      )}
    </div>
